    ...
    <div class="course-item bg-light">
      <div class="position-relative overflow-hidden">
        <NuxtLink :to="`/produkt/${product.slug}/`" :title="product.linkTitle">
          <nuxt-img preset="productThumbnail" class="img-fluid" :src="`${config.imageFolder}${product.localThumb}`"
      ...
      <div class="text-center p-4 pb-0">
        ...
        <div class="mb-4 h5">
          <nuxt-link :to="`/produkt/${product.slug}/`" class="noLinkStyle" :title="product.name">
            {{ product.name }}
          </nuxt-link>
        </div>
      </div>
      <div class="d-flex border-top">
        <small class="flex-fill text-center border-end py-2"><i class="fa fa-toilet text-primary me-2"></i>
          <NuxtLink :to="`/marken/${brandData.slug}/`" :title="decode(product.brand)" v-html="product.brand"></NuxtLink>
        </small>
        <small class="flex-fill text-center border-end py-2"><i class="fa text-primary me-2"></i>
          <NuxtLink :to="`/kategorie/${categoryData.slug}/`" :title="categoryData.name">
  ...
  data() {
    ...
    let categoryData = categories.find(
      (x) =>
        ...
        x.subCategories.find((y) => y.key == categoryName.toLowerCase()) || x.key === "sonstiges"
    );
